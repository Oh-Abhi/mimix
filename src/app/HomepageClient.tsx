...
import { getPublicCollections } from '@/lib/db'
import { DbCollection } from '@/lib/types'
import { useAuth } from '@/components/providers/AuthProvider'
import Link from 'next/link'
import { Music2, Heart, Play, Music } from 'lucide-react'
...
function CollectionCard({ col, index }: { col: DbCollection; index: number }) {
  ...
  const gradFrom = col.profiles?.theme === 'blush' ? '#831843' : col.profiles?.theme === 'paper' ? '#c7c2b8' : '#312e81'
  const gradTo = col.profiles?.theme === 'blush' ? '#4a044e' : col.profiles?.theme === 'paper' ? '#b8b2a4' : '#1e1b4b'
...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    getPublicCollections(32)
      .then(cols => { setCollections(cols); setLoading(false) })
    ...
    <div className="min-h-screen pt-28 pb-20 px-6 sm:px-10 lg:px-16">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-sm uppercase tracking-widest font-semibold" style={{ color: 'var(--text-muted)' }}>Public Collections</h2>
